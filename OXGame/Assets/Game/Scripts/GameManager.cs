...
public class GameManager : MonoBehaviour {
	...
	public int hiScore;
    public int Damage;
	public AudioSource audioSource;               //ref to audio source
    ...
    public Animator Party1Anim;
    public Animator Party2Anim;
    public Animator Party3Anim;
	...
	void Start()
	{
        ...
        answer = Answer.NotClicked;                     //at start the player input is nothing
        ...
        Party1Anim.SetTrigger("Stop");
        Party2Anim.SetTrigger("Stop");
        Party3Anim.SetTrigger("Stop");
	...
	public void TimerForQues()
	{
		...
		if (!isGameOver) {
            ...
            HpBar.value = BossHp / BossMaxHp;
        }
       
        if (timePerQues<=0 && answer==Answer.O)
        {
            if (currentQuestion.isTrue != "True")
            {
                BossAnim.SetTrigger("Attack");
                Party1Anim.SetTrigger("Die");
                Party2Anim.SetTrigger("Die");
                Party3Anim.SetTrigger("Die");
                isGameOver = true;
                audioSource.PlayOneShot(audioClips[1]);
            }
            else
            {
                Party1Anim.SetTrigger("Attack");
                Party2Anim.SetTrigger("Attack");
                Party3Anim.SetTrigger("Attack");
                timePerQues = 10;
                score++;
                BossHp = BossMaxHp - (Damage * score);
                audioSource.PlayOneShot(audioClips[0]);
            }
            if (BossHp <= 0)
            {
            ...
            if (isGameOver == false && BossHp>0)
            {
                StartCoroutine(TransitionToNextQuestion());
            }
        }

        if(timePerQues<=0 && answer==Answer.X)
        {
		...
		if (timePerQues <= 0 && answer==Answer.NotClicked ) 
		{
			audioSource.PlayOneShot (audioClips [1]);
			isGameOver = true;
		}
